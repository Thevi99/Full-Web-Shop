...
const resetIdentifier = async (req, res) => {
    try {
        ...
        // ตรวจสอบว่าอยู่ในสถานะ Cooldown หรือไม่
        if (user.Cooldown > Date.now()) {
            const timeLeft = user.Cooldown - Date.now();

            // คำนวณเวลาเหลือในชั่วโมงหรือในนาที
        ...
        // กำหนดเวลา Cooldown (เพิ่มเวลา 1 ชั่วโมงเป็นตัวอย่าง)
        const cooldownDuration = 1 * 60 * 60 * 1000; // 1 ชั่วโมงในมิลลิวินาที
        user.Cooldown = Date.now() + cooldownDuration;

        // ตั้งค่า Identifier ใหม่
        ...
        res.status(200).json({ message: 'Identifier reset successfully' });
    } catch (error) {
        console.error('Error resetting identifier:', error);
